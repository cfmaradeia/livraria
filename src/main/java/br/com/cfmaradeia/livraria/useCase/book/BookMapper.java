...
public class BookMapper {
    ...
    public Book bookFromRequestDTO(BookRequestDTO bookRequestDTO, Set<Author> authors, Set<Subject> subjects) {
        Book book = new Book();
    ...
    public BookResponseDTO responseDTOFromModel(Book book) {
        return new BookResponseDTO(
            ...
            book.getSubjects().stream().map(Subject::getId).collect(Collectors.toSet())
        );
    }
}
